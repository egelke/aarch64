...
Yes, I'm very well aware that the above code won't win any prizes (maybe with the exception of worst code).  The goal here isn't to write perfect C, it is to learn how to write aarch64 assembly.  That being said, what does it do?  Well it add 19 and 23, which give you 42 (the meaning of life if I recall correctly) and prints it out to the screen.
...
Why they have totally different beats me, but you will get used to it quite quickly.  While you can have multiple aliases for the same, you can't redefine alias within the same source file.  You will need to split up your sources in different files, use different names or simply don't use aliases.

## Memory

In the previous lessons we touched on the usage of memory, but didn't really explain it properly.  So lets do that now.

Basically all modern computers use the same memory for both data and code since [Von Neumann](https://en.wikipedia.org/wiki/Von_Neumann_architecture) described it being the best.  However, that doesn't mean we simply throw everything randomly together (as Von Neumman suggested), instead we organize it nicely in different section like this:
...
In the intro lesson we learned about __leaf function__, those are function that do not call any other function.  In the previous lesson we learned about __unchained non-leaf function__, those are functions that call other functions but don't take any action to properly record the chain of functions: they simply throw their stuff on the stack without indication of a frame boundary.  It entirely up to you, the developer, to pop everything back from the stack before returning.  In this lesson we focus on __Chained functions__, these functions follow certain conventions and standards.  These specify when and how to push stuff on the stack and to mark the boundary of the frame.  While it is still _mostly_ up to the code to pop everything from the stack before returning, there are advantages.  There are verification tools, it does allow debuggers to interpret the stack and Windows OS to do [Frame Base Exception Handling](https://docs.microsoft.com/en-us/windows/win32/debug/frame-based-exception-handling).
...
A function should manipulate the stack in the following manner:

* __prolog(ue)__: start of the functions, creates the "fixed" part of the stack frame, which consist of 3 sub-steps
  ...
  * allocate remaining local (stack) area with an additional 16 bytes and save the current values of `fp`/`lr`
  * mark the new boundary of the stack frame by copying `sp` register to the `fp` (alias for `x29`) register
* __function body__: the actual function code, which may include:
  ...
  * parameters 8 and beyond for outgoing function calls
* __epilog(ue)__: end of the function, freeing the stack frame & restoring registers in 3 sub-steps
  * freeing the dynamic allocated stack by copying the `fp` registers back to the `sp` register
  * free the local stack and restoring the `fp`/`lr` register pair
  * restore the non-volatile registers, which consist of 2 sub-steps
    * restore the floating point registers
    * restore the integer registers and free all remaining register memory

A few notes:

* You don't have to put saved registers on the stack unless you actually use them
* Home registers (`x0` to `x7`) do not need to be restored, you simply save the
...
* restoring `sp` from `fp` is only required if you updated the `sp` in the function body, i.e. dynamically extend the stack

[^1]: the size allocated in this step is therefor at least 0x10 bytes.

In the [source](main.asm) file you will find 2 examples.  The `_start` function is the most basic example possible, it saves no registers nor does it have any local area; it simply saves the `fp`/`lr` pair in the prolog and restores it in the epilog.  The `_printf` functions it a little more complicated, it uses 3 integer registers (`x19` to `x21`) and needs 0x88 bytes of local area.  In the first sub-step of the epilog it allocates 0x20 bytes for the saved registers because of the 0x10 (16) bytes alignment of the `sp` registers.  In the second sub-step it  allocations 0xA0, 0x88 rounded to the nearest 0x10 bytes plus an additional 0x10 for the `fp`/`lr` pair.
...
__Formalization__ was easy, there is a _canonical_ form of the prolog and epilog that is part of the Microsoft spec for (hardware) [exceptions](https://docs.microsoft.com/en-us/cpp/build/arm64-exception-handling).  It is basically what is described above, but can't update the `sp` after you copied to the `fp` (setting the `fp` is still required).  So no dynamic extension of the stack in the function body, but I wasn't planning doing that anyway.  Luckily ARM64 passes the first 8 parameters as registers, so there is very little need to use the stack to pass parameters which would require dynamic extension of the stack in the function body.

__Verification__ was a little harder, but came in the form of the `dumpbin` tool.  It can verify the prolog and epilog code of a functions if it has unwind data in packed format (see [here](https://docs.microsoft.com/en-us/cpp/build/arm64-exception-handling?view=msvc-160#packed-unwind-data)). I extended the `makefile` with the `unwindinfo` target for our convenience.
...
Most of them require parameters, like the first registry and/or the size of the memory. I basically had to take a guess what was expected, if I got it wrong; try again based on the error messages.

## Conclusion

We went a little deeper into functions, assembly directive and memory management.  With the exception of some very specific aspects of functions, we stayed rather close to the surface.  You should however be able to do most common tasks, dynamic heap allocation will come later.  There are definitely a lot off edge cases that aren't covered here.

In the next lesson we fill in the last gaps, after that we will be focusing on the aarch64 instructions itself.
