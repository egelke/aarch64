	...
	.macro cfunc_prolog, name:req, RegI=0, RegF=0, H=0, alloc=0
        ...
        //save the RegI
        .if \RegI % 2 == 1
            .error "only even RegIs are supported for now"
        .endif
        .if \RegI > 0
            stp x19, x20, [sp, #-((0x10 * \RegI / 2) + (0x10 * \RegF / 2))]!
            .seh_save_regp_x x19, (0x10 * \RegI / 2) + (0x10 * \RegF / 2)
        .endif
        .if \RegI > 2
            stp x21, x22, [sp, #0x10]
        ...
        //Save the RegF
        .if \RegF > 0
            .error "not possible due to bug in clang (issue #54900)"
        .endif
        .if \RegF > 0
    ...
    .macro cfunc_epilog, return=ret, RegI=0, RegF=0, H=0, alloc=0
        ...
        .endif
        .if \RegI > 0
            ldp x19, x20, [sp], #(0x10 * \RegI / 2) + (0x10 * \RegF / 2)
            .seh_save_regp_x x19, (0x10 * \RegI / 2) + (0x10 * \RegF / 2)
        .endif
